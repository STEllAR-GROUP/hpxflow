...
namespace hpx
{
  ...
  namespace flow
  {
    ...
    template < class TypeVal > class AtomicCounter
    {
    public:
	...
	if (this == &other)
	  return *this;
	boost::mutex::scoped_lock lock1 (&m_atomicMutex <
					 &other.
					 m_atomicMutex ? m_atomicMutex :
					 other.m_atomicMutex);
	boost::mutex::scoped_lock lock2 (&m_atomicMutex >
					 &other.
					 m_atomicMutex ? m_atomicMutex :
					 other.m_atomicMutex);
    ...
    public:
      ...
      {
	boost::unique_lock < boost::mutex > (const_cast <
					     boost::mutex & >(cpy.
							      m_cdlMutex));
	...
	if (this == &other)
	  return *this;
	boost::mutex::scoped_lock lock1 (const_cast <
					 boost::mutex & >(&m_cdlMutex <
							  &other.
							  m_cdlMutex ?
							  m_cdlMutex : other.
							  m_cdlMutex));
	boost::mutex::scoped_lock lock2 (const_cast <
					 boost::mutex & >(&m_cdlMutex >
							  &other.
							  m_cdlMutex ?
							  m_cdlMutex : other.
	...
	if (m_cdlCount.getCount () > 0)
	  {
	    boost::posix_time::time_duration td =
	      boost::posix_time::milliseconds (timeoutMicros);
